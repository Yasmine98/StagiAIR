...
// /E/image?id=123
@WebServlet(urlPatterns = { "/Rapports" })
public class DisplayRapportServlet extends HttpServlet {
  private static final long serialVersionUID = 1L;
 @PersistenceContext(unitName = "StagiAIResAlgerie-warPU")
 EntityManager em;
  public DisplayRapportServlet() {
      super();
  }
 
  private Stage getRapportInTable(Connection conn, Long id) throws SQLException {
     
      Stage stage = em.find(Stage.class, 8);
      ...
      try {
         ...
         */
         String para = request.getParameter("id");
          ...
          stage = em.find(Stage.class, i);
      System.out.println(stage.getId());
          if (stage == null) {
              // No record found, redirect to default image.
              response.sendRedirect(request.getContextPath() + "/Rapports/noimage.jpg");
              return;
          }
        
          // trump.jpg, putin.png
          String rapportFileName = stage.getIdtheme().getNom();
          System.out.println("File Name: "+ rapportFileName);
        
          // image/jpg
          // image/png
          String contentType = this.getServletContext().getMimeType(rapportFileName);
          System.out.println("Content Type: "+ contentType);
        System.out.println("Content-Length: " + String.valueOf(stage.getRapport().length));
          response.setHeader("Content-Type", contentType);
        
          response.setHeader("Content-Length", String.valueOf(stage.getRapport().length));
        
          response.setHeader("Content-Disposition", "inline; filename=\"" + stage.getIdtheme().getNom()+ "\"");
 
          // Write image data to Response.
          response.getOutputStream().write(stage.getRapport());
